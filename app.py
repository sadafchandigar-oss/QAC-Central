...
def log_activity(mill_id, action, details):
    conn = get_db()
    conn.execute("INSERT INTO activity_log (mill_id, action, details) VALUES (?,?,?)", (mill_id, action, details))
    conn.commit()
    conn.close()
...
def dashboard():
    ...
    files_month = conn.execute("SELECT COUNT(*) FROM files WHERE strftime('%Y-%m', uploaded_at)=strftime('%Y-%m','now','localtime')").fetchone()[0]
    recent_activity = conn.execute("SELECT a.*, m.name as mill_name FROM activity_log a LEFT JOIN mills m ON a.mill_id=m.id ORDER BY a.done_at DESC LIMIT 10").fetchall()
    conn.close()
    return render_template('dashboard.html', mills=mills, total_mills=total_mills, total_projects=total_projects, pending=pending, files_month=files_month, recent_activity=recent_activity)
...
def mills_list():
    ...
    conn = get_db()
    if q:
        mills = conn.execute("SELECT * FROM mills WHERE name LIKE ? OR location LIKE ? OR mill_type LIKE ? ORDER BY name", (f'%{q}%', f'%{q}%', f'%{q}%')).fetchall()
    else:
        mills = conn.execute("SELECT * FROM mills ORDER BY name").fetchall()
    mills_data = []
    for m in mills:
    ...
    if request.method == 'POST':
        ...
        conn.execute("INSERT INTO mills (name, location, mill_type, contact_person, contact_phone, notes) VALUES (?,?,?,?,?,?)",
                     (name, request.form.get('location'), request.form.get('mill_type'),
                      request.form.get('contact_person'), request.form.get('contact_phone'), request.form.get('notes')))
        conn.commit()
...
def mill_detail(mill_id):
    ...
    conn.close()
    return render_template('mill_detail.html', mill=mill, files_tech=files_tech, files_draw=files_draw, files_prod=files_prod, projects=projects, activity=activity)
    ...
    if request.method == 'POST':
        conn.execute("UPDATE mills SET name=?, location=?, mill_type=?, contact_person=?, contact_phone=?, notes=? WHERE id=?",
                     (request.form['name'], request.form.get('location'), request.form.get('mill_type'),
                      request.form.get('contact_person'), request.form.get('contact_phone'), request.form.get('notes'), mill_id))
        conn.commit()
...
def upload_file(mill_id):
    ...
    for f in files:
        if f.filename:
            fname = f.filename
            existing = conn.execute("SELECT MAX(version) FROM files WHERE mill_id=? AND department=? AND filename=?", (mill_id, dept, fname)).fetchone()[0]
            version = (existing or 0) + 1
            result = cloudinary.uploader.upload(f, folder=f"qac/{mill_id}/{dept}", resource_type="auto", public_id=f"{fname}_v{version}")
            conn.execute("INSERT INTO files (mill_id, department, filename, cloudinary_url, cloudinary_id, version, notes) VALUES (?,?,?,?,?,?,?)",
                         (mill_id, dept, fname, result['secure_url'], result['public_id'], version, notes))
...
def delete_file(file_id):
    ...
    if f:
        try:
            cloudinary.uploader.destroy(f['cloudinary_id'], resource_type="raw")
        except:
            pass
        mill_id = f['mill_id']
...
def add_project(mill_id):
    ...
    conn.execute("INSERT INTO projects (mill_id, title, status, department, description) VALUES (?,?,?,?,?)",
                 (mill_id, request.form['title'], request.form.get('status', 'Pending'), request.form.get('department'), request.form.get('description')))
    conn.commit()
...
def activity_log():
    conn = get_db()
    logs = conn.execute("SELECT a.*, m.name as mill_name FROM activity_log a LEFT JOIN mills m ON a.mill_id=m.id ORDER BY a.done_at DESC LIMIT 100").fetchall()
    conn.close()
...
def search():
    q = request.args.get('q', '')
    conn = get_db()
    mills = conn.execute("SELECT * FROM mills WHERE name LIKE ? OR location LIKE ?", (f'%{q}%', f'%{q}%')).fetchall() if q else []
    files = conn.execute("SELECT f.*, m.name as mill_name FROM files f JOIN mills m ON f.mill_id=m.id WHERE f.filename LIKE ? ORDER BY f.uploaded_at DESC", (f'%{q}%',)).fetchall() if q else []
    conn.close()
